            ...
            if not self.lUpdated[indx]:
                try:
                    ...
                    datestr = '%s-%s-%s' %(str(now.year)[-2:],
                        str('{:02d}'.format(now.month)), str('{:02d}'.format(now.day)))
                    filePath = '%s/%s/maxigauge %s.log' %(logFolderPath, datestr, datestr)
                    # read all values from text file
                    with open(filePath, 'rb') as f:
                        data = f.readlines()
                        lastline = data[-1].split(b',')
                        self.lValues = [float(lastline[5]), float(lastline[11]), float(lastline[17]),
                                        float(lastline[23]), float(lastline[29]), float(lastline[35])]
